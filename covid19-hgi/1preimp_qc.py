...
def read_input(dirname, basename, reference, phenofile, inputType) -> hl.MatrixTable:
    ...
    elif inputType == "vcfgz":
        mt = hl.import_vcf(dirname + basename + ".vcf.gz",
                           force_bgz=True,
                           contig_recoding={f"{i}":f"chr{i}" for i in (list(range(1, 23)) + ['X'])},
                           skip_invalid_loci=True)

        mt = mt.repartition(n_partitions=100, shuffle=True)

        mt = mt.annotate_cols(is_female=(hl.missing(hl.tbool)))

        mt.write(dirname + 'qc_step_1/' + basename + '.mt', overwrite=True)

        mt = hl.read_matrix_table(dirname + 'qc_step_1/' + basename + ".mt")
...
def remove_related_samples(mt, dirname):
    ...
    samples_list = samples_to_remove.annotate(sample_to_remove=hl.cond(
        samples_to_remove.cr_s1 >= samples_to_remove.cr_s2, samples_to_remove.j, samples_to_remove.i))
...
def qc_filter(dirname, basename, reference, maf, sample_miss, snp_miss, m_f_thr, phenofile, inputType):
    ...
    mt = mt.filter_entries(mt.GT.is_diploid())
...
def main():
    ...
    parser.add_argument('--inputType', type=str, required=True, choices=['hail','plink','vcfgz'])
