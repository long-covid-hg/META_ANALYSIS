...
from hail import hadoop_open  # for writing the qc-filter.log file to google bucket


def read_input(dirname, basename, reference, phenofile, inputType):
    if inputType == "plink":
        ...
        mt.write(dirname + basename + '.plink.mt', overwrite=True)

        mt = hl.read_matrix_table(dirname + basename + ".plink.mt")
    ...
    #mt.write(dirname + basename + '.input.mt', overwrite=True)
    # hl.export_plink(mt, 'gs://dsge-covid19-data/cov_illumina_14082020/plus_cov_illumina_14082020_QC_mc',
...
def recode_allele_N(dirname, basename, reference, phenofile, inputType):
    """
...
def pre_qc(dirname, basename, reference, phenofile, inputType):
    ...
    :param mt: MatrixTable before pre_QC
    ...
    # Remove contigs other than chr1-22, X (some data contains contig like 'chr1_KI270706v1_random': 4, 'chr8_KI270821v1_alt': 12,)
    ...
    n_females = mt.filter_cols(mt.is_female == True).s.collect()
    n_males = mt.filter_cols(mt.is_female == False).s.collect()
    n_sex_missing = mt.filter_cols(hl.is_missing(mt.is_female)).s.collect()
    n_cases = mt.filter_cols(mt.is_case == True).s.collect()
    n_controls = mt.filter_cols(mt.is_case == False).s.collect()
    n_unknown_pheno = mt.filter_cols(hl.is_missing(mt.is_case)).s.collect()

    print("<============== Sample and SNP summary (pre-qc) ==============>")
    print("{} SNPs to be included in pre-QC filtering".format(initial_vars))
    print("{} samples to be included in pre-QC filtering".format(initial_samples))
    print("Number of females: {}".format(len(n_females)))
    print("Number of males: {}".format(len(n_males)))
    print("Number of samples with unspecified sex: {}".format(len(n_sex_missing)))
    print("Number of cases: {}".format(len(n_cases)))
    print("Number of controls: {}".format(len(n_controls)))
    print("Number of samples with unknown phenotype: {}".format(len(n_unknown_pheno)))
    ...
    print("Number of SNPs on contigs chr1-X: {}\t Filtered SNPs: {}"
          .format(snps_filt_contigs, (initial_vars - snps_filt_contigs)))
    ...
    print("Total SNPs removed: {}".format(initial_vars - snps_filt_call))
...
def get_samples_to_remove(mt, maf, miss, mfthr, dirname):
    ...
    if len(discordant_sex_cases) > 0:
        imputed_sex.filter(hl.literal(discordant_sex_cases).contains(imputed_sex['s'])).export(
            dirname + 'imputed_sex_discordant.tsv')
...
def remove_related_samples(mt, dirname):
    ...
    high_ld_regions = hl.import_locus_intervals('gs://dsge-covid19-data/high-ld-regions_b38.tsv', reference_genome='GRCh38')
    ...
    #print("Number of variants remaining after filtering out multiallelic variants: {}".format(biallelic_dataset.count_rows()))
    #print("Number of variants remaining after filtering out variants in LD: {}".format(mt.count_rows()))
    ...
    samples_to_remove = samples_to_remove.annotate(cr_s1 = call_rate_dict[samples_to_remove.i], cr_s2 = call_rate_dict[samples_to_remove.j])
    samples_list = samples_to_remove.annotate(sample_to_remove=hl.cond(samples_to_remove.cr_s1 >= samples_to_remove.cr_s2, samples_to_remove.i, samples_to_remove.j))
    samples = samples_list.sample_to_remove.collect()

    if len(samples) > 0:
        with hadoop_open(dirname + 'relatedness_removed_samples.tsv', 'w') as f:
...
def qc_filter(dirname, basename, reference, maf, sample_miss, snp_miss, m_f_thr, phenofile, inputType):
    ...
    sample_missingness, sample_sex_check_controls,  sample_sex_check_cases = get_samples_to_remove(mt, maf, sample_miss, m_f_thr,
                                                                          dirname)
    ...
    with hadoop_open(dirname + 'qc-filter.log', 'w') as f:
        f.write(pre_qc_log)
        sys.stdout = f
        print("\n<============== Sample-QC ==============>")
        ...
        print("Samples removed for relatedness check: " + str(len(related_samples)))
    ...
    mt.write(dirname + basename + '.qc.mt', overwrite=True)
...
def main():
    ...
    parser.add_argument('--maf', default=0.01)
    parser.add_argument('--sampleMiss', default=0.02)
    parser.add_argument('--snpMiss', default=0.02)
