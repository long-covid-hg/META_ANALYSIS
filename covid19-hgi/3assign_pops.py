...
def load_merge_data(ref_scores, ref_info, data_scores):
...
def pca_plot(pcs_data: str = None, ref_data: str = None, ref_info_data: str = None, include_ref: bool = None,
             cohort: str = None, out_dir: str = None, prob: float = None, phenotypes_file: str = None,
             pheno: str = None):
    ...
    ref_update = pd.merge(left=ref, right=ref_info, left_on='s', right_on='Sample', how='inner')
    ...
    if include_ref is True:
        ...
        if cohort:
            plt_title = f'{cohort} projected against 1KG+HGDP (p > {prob})'
            outfile = f'{cohort}.1KG.PC1-PC6.prob_{prob}.png'
        else:
            plt_title = f'All cohorts projected against 1KG+HGDP (p > {prob})'
    ...
    for i in range(1, 6):
        ...
        if include_ref:
            axs[axisx, axisy].scatter(ref_update[f'PC{i}'], ref_update[f'PC{i + 1}'],
                                      c=ref_update['SuperPop'].map(cbPalette),
                                      s=5, alpha=0.1)
        axs[axisx, axisy].scatter(pcs_df[f'PC{i}'], pcs_df[f'PC{i + 1}'], c=pcs_df['pop'].map(cbPalette), s=15, alpha=1, marker = "X")
...
def main(args):
    ...
    ref_scores = args.dirname + 'qc_step_2/1KG_HGDP_scores.txt.bgz'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ref_info', default='gs://dsge-covid19-data/1KG_HGDP/hgdp_1kg_sample_info.tsv')
...
# --ref_scores gs://lindo/covid19_0206/qc_step_2/1KG_HGDP_scores.txt.bgz
# --data_scores gs://lindo/covid19_0206/qc_step_2/PLUS_COV_ILLUMINA_02062021.chr0.pos0.4samplesGER.removed.qc_scores.tsv
# --dirname gs://lindo/covid19_0206/
# --phenotype_file gs://dsge-covid19-data/phenotypes_main_2021_06_16.tsv
